...
function click_status(str){
    var table = $(str);
    if (table.hasClass('d-none')) table.removeClass('d-none');
    else table.addClass('d-none');
}

function history_request(symbol){
    $.ajax({
        type: "GET",
        contentType: "application/json; charset=utf-8",
        url: 'stock/history?symbol=' + symbol,
        dataType: 'json',
        success: function (data) {
            histroy_chart(data);
        },
...
function histroy_chart(data){
    ...
    d3.select('#history_chart').selectAll('g').remove();

    var svg = d3.select('#history_chart')
    ...
    y.domain([0, d3.max(data.data, d => d.open)]);

    svg.append("path")
      .data([data.data])
      .attr("class", "line")
      .attr("d", valueline).attr('style', "fill:none;stroke:#2C3E50");
